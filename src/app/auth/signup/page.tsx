...
export default function SignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // 2 — Create the couple record via our API (server handles DB write securely)
    const res = await fetch('/api/couples', { method: 'POST' });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      setError(body.error ?? 'Failed to create your space. Please try again.');
      setLoading(false);
      return;
    }

    // 3 — Onboarding wizard to fill in couple details
    router.push('/setup');
    router.refresh();
  }
  ...
  async function handleGoogle() {
    ...
    const supabase = createClient();
    // Callback will create the couple + redirect to /setup for new users
    ...
    >
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-serif text-4xl text-white mb-2">Create your space</h1>
          <p className="font-sans text-sm" style={{ color: 'rgba(255,255,255,0.5)' }}>
            Your couple's private corner of the internet
          </p>
        </div>
        ...
        <div className="bg-cream rounded-3xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            {error && (
              <p className="text-sm font-sans text-rose-deep text-center">{error}</p>
            )}
          ...
          <p className="font-sans text-sm text-center text-ink-light mt-6">
            Already have an account?{' '}
            <Link href="/auth/login" className="text-rose-deep underline underline-offset-2">
              Sign in
            </Link>
          </p>
        </div>
